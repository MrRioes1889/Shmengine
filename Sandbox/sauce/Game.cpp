...
bool32 game_update(Game* game_inst, float32 delta_time) 
{
    ...
    if (Input::key_pressed(Keys::KEY_T))
    {
        ...
        Event::event_fire(EVENT_CODE_DEBUG0, 0, {});
    }    

    if (Input::is_key_down(Keys::KEY_A) || Input::is_key_down(Keys::KEY_LEFT))
    {
        camera_yaw(state, 1.0f * delta_time);
    }
    if (Input::is_key_down(Keys::KEY_D) || Input::is_key_down(Keys::KEY_RIGHT))
    {
        camera_yaw(state, -1.0f * delta_time);
    }
    ...
    if (Input::is_key_down(Keys::KEY_E))
    {
        velocity.x = temp_move_speed * delta_time;
    }
    if (Input::is_key_down(Keys::KEY_Q))
    {
        velocity.x = -temp_move_speed * delta_time;
    }
