...
namespace ResourceSystem
{
	...
	bool32 material_loader_load(const char* name, void* params, MaterialConfig* out_config)
	{
		...
		CString::copy(name, out_config->name, max_material_name_length);
        ...
        while (FileSystem::read_line(file_content.c_str(), line, &continue_ptr))
        {
            ...
            {
                CString::copy(value.c_str(),out_config->name, max_material_name_length);
            }
            ...
            {
                CString::copy(value.c_str(), out_config->diffuse_map_name, max_texture_name_length);
            }
            else if (var_name.equal_i("specular_map_name"))
            {
                CString::copy(value.c_str(), out_config->specular_map_name, max_texture_name_length);
            }
            else if (var_name.equal_i("normal_map_name"))
            {
                CString::copy(value.c_str(), out_config->normal_map_name, max_texture_name_length);
            }
