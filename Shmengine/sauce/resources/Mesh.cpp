...
static void mesh_load_job_success(void* params) {
    ...
    if (mesh->resource_name)
        SHMTRACEV("Successfully loaded mesh '%s'.", mesh->resource_name);
    else
        SHMTRACE("Successfully loaded nameless mesh.");
...
static void mesh_load_job_fail(void* params) {
    ...
    SHMERRORV("Failed to load mesh '%s'.", mesh->resource_name);
...
static bool32 mesh_load_job_start(void* params, void* result_data) {
    ...
    bool32 result;
    if (!load_params->is_reload && mesh->resource_name)
        result = ResourceSystem::mesh_loader_load(mesh->resource_name, 0, &load_params->mesh_resource);
    else
        result = true;
