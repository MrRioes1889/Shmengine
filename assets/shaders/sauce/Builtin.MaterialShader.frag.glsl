...
void main()
{
    ...
    if(in_mode == 0 || in_mode == 1) 
    {
        ...
        out_color = calc_dir_lighting(object_ubo.dir_light, normal, view_direction);

        for (uint i = 0; i < object_ubo.p_lights_count; i++)
        {
            out_color += calc_point_lighting(object_ubo.p_lights[i], normal, in_dto.frag_position, view_direction);
        }
